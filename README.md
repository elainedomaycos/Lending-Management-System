...
- Vercel deploy: the API is served as a serverless function from `api/index.mjs` (pre-built bundle), configured in `artifacts/api-server/vercel.json`. `vercel.json` sets `"framework": null` (preset "Other") so Vercel does not run its zero-config Express build (which would `tsc`-emit `src/**` and fail with "Emit skipped"); the only function is the pre-built `api/index.mjs`. Set `DATABASE_URL` in the Vercel project env.
